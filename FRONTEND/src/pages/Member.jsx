        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-gray-50 rounded-xl p-3 text-center">
              <p className="text-2xl font-black text-indigo-600">{member.member_count}</p>
          ...
          <div>
            ...
            <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
              <div className={`h-full rounded-full transition-all ${isEligible ? "bg-emerald-500" : "bg-indigo-500"}`}
                style={{ width: `${Math.min((member.member_count / threshold) * 100, 100)}%` }}/>
            </div>
          </div>
          <div className={`rounded-xl px-3 py-2.5 text-xs font-semibold text-center ${isEligible ? "bg-emerald-50 text-emerald-700 border border-emerald-100" : "bg-gray-50 text-gray-400"}`}>
            {isEligible ? "🎁 Layak mendapat reward!" : `Butuh ${threshold - member.member_count} kunjungan lagi`}
          </div>
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col">
      ...
      <div className="flex gap-2 px-4 py-3 border-t border-gray-100">
        <button
          ...
          disabled={printing}
          className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-emerald-500 text-white text-xs font-semibold hover:bg-emerald-600 active:scale-95 transition-all shadow-sm shadow-emerald-200 disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {printing ? (
            <>
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col">
      <div className="px-4 pt-4 pb-2">
        ...
        <div className="flex items-end gap-3 mb-1">
          ...
          <div className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold mb-1 ${growthPct >= 0 ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-500"}`}>
            {growthPct >= 0 ? "↑" : "↓"} {Math.abs(growthPct)}%
          </div>
        </div>
        <div className="flex items-center gap-4 mb-3">
          <div className="text-center">
            <p className="text-sm font-black text-emerald-600">{eligible}</p>
            <p className="text-xs text-gray-400">Layak reward</p>
          </div>
          <div className="w-px h-8 bg-gray-100" />
          <div className="text-center">
            <p className="text-sm font-black text-indigo-600">{BISNIS.reward_threshold}</p>
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
        ...
        <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2.5 py-1 rounded-full">{members.length} member</span>
      </div>
      ...
      <div className="flex gap-2 px-4 py-3 border-b border-gray-100">
        <div className="flex-1 relative">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Cari nomor HP…"
            className="w-full pl-8 pr-3 py-2 rounded-xl border border-gray-200 bg-gray-50 text-xs outline-none focus:border-indigo-400 focus:bg-white transition-all"/>
        </div>
        <button onClick={() => setFilterReward(f => !f)}
          className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold border transition-all flex-shrink-0 ${filterReward ? "bg-emerald-500 text-white border-emerald-500" : "border-gray-200 text-gray-500 hover:border-gray-300"}`}>
          🎁 Reward
        </button>
      </div>
            ...
            {filtered.length === 0 ? (
              ...
              return (
                <tr key={m.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-3 py-3 text-center">
                    <div className="inline-flex flex-col items-center">
                      ...
                      <div className="w-10 h-1 bg-gray-100 rounded-full overflow-hidden mt-1">
                        <div className={`h-full rounded-full ${isEligible ? "bg-emerald-500" : "bg-indigo-400"}`}
                  ...
                  {/* Badge */}
                  <td className="px-3 py-3 text-center">
                    <span className={`inline-block px-2 py-1 rounded-full text-xs font-bold ${isEligible ? "bg-emerald-50 text-emerald-600 border border-emerald-100" : "bg-gray-100 text-gray-400"}`}>
                      {isEligible ? "🎁 Reward" : "Proses"}
                    </span>
                  </td>
                  {/* Aksi */}
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-100 px-4 lg:px-8 pt-10 lg:pt-6 pb-4">
        <p className="text-xs font-semibold text-emerald-500 uppercase tracking-widest mb-0.5">Membership</p>
