...
function Input({ error, ...props }) {
    return (
        ...
        ${error ? "border-red-300 bg-red-50" : "border-gray-200 bg-gray-50 focus:bg-white focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"}`}
        />
    );
}
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
            ...
            <div className="relative w-full sm:max-w-lg bg-white sm:rounded-3xl rounded-t-3xl shadow-2xl overflow-hidden max-h-[92vh] flex flex-col">
                ...
                <div className="flex gap-3 px-5 py-4 border-t border-gray-100 flex-shrink-0">
                    ...
                    <button onClick={onSave} className="flex-1 py-2.5 rounded-xl text-sm font-semibold bg-indigo-600 text-white hover:bg-indigo-700 active:scale-95 transition-all">{saveLabel}</button>
                </div>
            </div>
        </div>
    );
                ...
                <div className="grid grid-cols-2 gap-2">
                    {["barang", "jasa"].map(t => (
                        <button key={t} type="button" onClick={() => set("bisnis_tipe", t)}
                            className={`py-2.5 rounded-xl border-2 text-sm font-medium capitalize transition-all ${form.bisnis_tipe === t ? "border-indigo-500 bg-indigo-50 text-indigo-700" : "border-gray-200 text-gray-500"}`}>
                            {t === "barang" ? "🛍️ Barang" : "🛠️ Jasa"}
                        </button>
                    ))}
                </div>
            </Field>
            ...
            <Field label="Jumlah Pegawai">
                <div className="flex items-center gap-3">
                    <button type="button" onClick={() => set("jumlah_pegawai", Math.max(1, form.jumlah_pegawai - 1))}
                        className="w-10 h-10 rounded-xl border-2 border-gray-200 text-gray-600 text-lg font-bold hover:border-indigo-300 transition-all">−</button>
                    ...
                    {TUJUAN_OPTIONS.map(t => {
                        ...
                        return (
                            <button key={t} type="button" onClick={() => toggleTujuan(t)}
                                className={`w-full flex items-center gap-3 px-3.5 py-3 rounded-xl border-2 text-left transition-all ${active ? "border-indigo-400 bg-indigo-50" : "border-gray-200 bg-white"}`}>
                                <div className={`w-4 h-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${active ? "border-indigo-500 bg-indigo-500" : "border-gray-300"}`}>
                                ...
                                <span className={`text-xs font-medium ${active ? "text-indigo-700" : "text-gray-600"}`}>{t}</span>
                            </button>
                        );
                    })}
                </div>
    ...
    return (
        <Modal title="Edit Produk" subtitle={produk.produk_nama} onClose={onClose} onSave={() => onSave(form)}>
            ...
            <div className="flex items-center gap-4">
                <div className="w-20 h-20 rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50 flex items-center justify-center overflow-hidden flex-shrink-0 cursor-pointer hover:border-indigo-300 transition-all"
                ...
                <div>
                    ...
                    <button onClick={() => document.getElementById("img-upload").click()}
                        className="mt-2 text-xs text-indigo-600 font-semibold hover:text-indigo-800">{preview ? "Ganti Foto" : "Upload Foto"}</button>
                </div>
        ...
        <Modal title="Tambah Produk" onClose={onClose} onSave={() => { if (validate()) onSave({ ...form, id: Date.now() }); }}>
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <button onClick={() => document.getElementById("img-upload-new").click()} className="mt-2 text-xs text-indigo-600 font-semibold hover:text-indigo-800">Upload</button>
                </div>
...
function TabBisnis({ bisnis, onEdit }) {
    const INFO = [
        { label: "Tipe Bisnis", value: bisnis.bisnis_tipe === "jasa" ? "🛠️ Jasa" : "🛍️ Barang" },
        ...
        <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                    ...
                    <button onClick={onEdit} className="flex items-center gap-1 text-xs font-semibold text-indigo-600 hover:text-indigo-800 transition-colors">
                ...
                <div className="divide-y divide-gray-50">
                    {INFO.map(r => (
                        <div key={r.label} className="flex items-start justify-between px-4 py-3 gap-3">
                            <span className="text-xs text-gray-400 flex-shrink-0 mt-0.5">{r.label}</span>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
                ...
                <div className="flex flex-wrap gap-2">
                    {bisnis.tujuan_bisnis.map(t => (
                        <span key={t} className="text-xs font-medium px-3 py-1.5 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-100">{t}</span>
                    ))}
                </div>
            </div>
        </div>
    ...
    return (
        <div className="space-y-3">
            {produkList.map(p => (
                <div key={p.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm font-semibold text-indigo-600 mt-0.5">Rp {fmt(p.produk_harga)}</p>
                        {p.net_profit_margin != null && (
                            <span className="inline-block mt-1 text-xs px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-600 font-semibold border border-emerald-100">
                    ...
                    {/* Edit */}
                    <button onClick={() => onEdit(p)} className="w-8 h-8 rounded-xl bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-indigo-50 hover:text-indigo-600 transition-all flex-shrink-0">
            ...
            <button onClick={onAdd}
                className="w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl border-2 border-dashed border-indigo-300 text-indigo-600 text-sm font-semibold hover:bg-indigo-50 hover:border-indigo-400 transition-all">
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="grid grid-cols-3 gap-2.5">
                {[
                    { label: "Pendapatan", value: totalPendapatan, color: "text-indigo-600", bg: "bg-indigo-50" },
                    { label: "Pengeluaran", value: totalPengeluaran, color: "text-red-500", bg: "bg-red-50" },
                    { label: "Laba", value: totalLaba, color: "text-emerald-600", bg: "bg-emerald-50" },
                ].map(s => (
            ...
            <div className="bg-white border-b border-gray-100 px-4 lg:px-8 pt-10 lg:pt-8 pb-0">
                <div className="max-w-2xl lg:mx-0">
                    ...
                    <div className="flex items-center gap-4 mb-5">
                        <div className="w-16 h-16 rounded-2xl bg-indigo-600 flex items-center justify-center text-white text-xl font-black flex-shrink-0 shadow-sm shadow-indigo-200">
                        ...
                        <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-1.5 mt-1.5">
                                <span className="text-xs px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-600 font-semibold border border-indigo-100 capitalize">
                                    {bisnis.bisnis_tipe === "jasa" ? "🛠️" : "🛍️"} {bisnis.bisnis_tipe}
                                </span>
                    ...
                    <div className="flex gap-0">
                        {TABS.map(t => (
                            <button key={t.key} onClick={() => setTab(t.key)}
                                className={`flex items-center gap-1.5 px-4 py-2.5 text-xs font-semibold border-b-2 transition-all ${tab === t.key ? "border-indigo-600 text-indigo-600" : "border-transparent text-gray-400 hover:text-gray-600"
                                    }`}>
